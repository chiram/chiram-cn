...
import { E as createHeadAndContent, H as generateCspDigest, O as unescapeHTML, a as renderScriptElement, d as renderComponent, i as spreadAttributes, o as renderUniqueStylesheet, y as renderTemplate } from "./jsx-runtime_CXXmkpIb.mjs";
import { $ as AstroError, U as RenderUndefinedEntryError, Y as UnknownContentCollectionError } from "./errors-data_BVsUkJU2.mjs";
...
var ImmutableDataStore = class ImmutableDataStore {
	...
	*/
	static async fromModule() {
		try {
			const data = await import("./_astro_data-layer-content_DKZ3KHOn.mjs");
...
function createGetCollection({ liveCollections }) {
	return async function getCollection(collection, filter) {
		...
		if (await store.hasCollection(collection)) {
			const { default: imageAssetMap } = await import("./content-assets_D5PYLusD.mjs");
			const result = [];
...
async function updateImageReferencesInBody(html, fileName) {
	...
	const { getImage } = await import("./_virtual_astro_get-image_CiTxop19.mjs");
...
async function renderEntry(entry) {
	...
	if (entry.deferredRender) try {
		const { default: contentModules } = await import("./content-modules_CX1pWSHo.mjs");
...
//#endregion
export { renderEntry as n, getCollection as t };
